...
/**
 * Class Name: StoreGroupMessageReceiver
 * Class Description: 存储群聊消息接收器
 *
 * @author liujm
 * @date 2023-04-01
 */
...
public class StoreGroupMessageReceiver {
    ...
    @RabbitListener(
            bindings = @QueueBinding(
                    ...
                    exchange = @Exchange(value = Constants.RabbitConstants.STORE_GROUP_MESSAGE, type = "topic", durable = "true")
            ),
            concurrency = "1"
    )
    public void onChatMessage(@Payload Message message, @Header Map<String, Object> headers, Channel channel) throws IOException {
        ...
        try {
            ...
            DoStoreGroupMessageDTO storeGroupMessageDTO = jsonObject.toJavaObject(DoStoreGroupMessageDTO.class);
            storeGroupMessageDTO.setImMessageBodyEntity(messageBody);
            storeMessageService.doStoreGroupMessage(storeGroupMessageDTO);
